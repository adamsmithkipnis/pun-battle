...
SERVICES=(com.puntime.bot com.puntime.deploy)
...
status_report() {
  ...
  for service in com.puntime.bot com.puntime.deploy; do
    local line pid
...
[ -f "$REPO_DIR/main.py" ] || die "run this from the pun-time checkout"
say "$REPO_DIR"
...
if [ ! -f "$REPO_DIR/.env" ]; then
  if [ "$DRY_RUN" -eq 0 ]; then
    ...
    /usr/bin/sed -i '' \
      -e "s|^DB_PATH=.*|DB_PATH=$REPO_DIR/puntime.db|" \
      -e "s|^LOG_PATH=.*|LOG_PATH=$REPO_DIR/puntime.log|" \
      "$REPO_DIR/.env"
...
for service in "${SERVICES[@]}"; do
  ...
  tmp="$(mktemp)"
  /usr/bin/sed "s|/ABSOLUTE/PATH/TO/pun-time|$REPO_DIR|g" "$template" > "$tmp"
