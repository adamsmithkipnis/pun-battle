...
(on the Mac Mini, at `/Users/robot/pun-time`). The Mini's checkout is also the
...
```bash
launchctl list | grep puntime                        # is it running
launchctl kickstart -k gui/$(id -u)/com.puntime.bot  # restart
tail -f puntime.log                                  # watch it
